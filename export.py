...
def export_model_for_android(model, food_tags, output_prefix="food_model"):
    """导出模型和配置供安卓使用"""
    ...
    with open(f"{output_prefix}_vectors.json", "w", encoding="utf-8") as f:
    ...
    with open(f"{output_prefix}_config.json", "w", encoding="utf-8") as f:
        json.dump(food_config, f, ensure_ascii=False, indent=2)

    print(f"已导出安卓配置文件:")
    print(f"  - {output_prefix}_vectors.json")
...
# 2. 导出为二进制格式（更小，加载更快）
def export_binary_vectors(model, output_file="food_vectors.bin"):
    """导出二进制词向量文件"""
